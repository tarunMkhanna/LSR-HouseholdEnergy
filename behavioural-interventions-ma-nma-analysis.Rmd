...
today <- Sys.Date()
docfile <- paste0("study_information_table_", today, ".doc")
htmlfile <- paste0("study_information_table_", today, ".html")

save_as_html(ft, path = htmlfile)
...
ggsave("20250613-results/smd_percentagechange_density.png")
```
...
dfinc_all <- dfinc_all %>%
  ...
  filter(!(intervention %in% treats_to_remove$intervention)) # %>%
# 565 observations
...
model_reml <- rma.mv(smd, var_smd, random = ~ 1| as.factor(documentID)/as.factor(effectID), data = dfinc_all, method = "REML",
                                             mods = ~ Feedback:Information:SocialComparison:Motivation:MonetaryRewards+0)
# compute I^2
compute_I2(model_reml)

EffectSizeSummary_RE <- data_frame() 
EffectSizeSummary_RE <- rbind(EffectSizeSummary_RE, tidy3(model_reml))
EffectSizeSummary_RE<- separate(EffectSizeSummary_RE, model, sep = "[^[:alnum:]]+", into = c("Feedback", "Information", "SocialComparison", "Motivation", "MonetaryRewards"))

EffectSizeSummary_RE <- recode_binary_predictors(EffectSizeSummary_RE)
EffectSizeSummary_RE <- process_order_label(EffectSizeSummary_RE)
```

```{r meta-analysis correcting for publication bias}
# MA (Comparing interventions in a meta regression model and interactions)

model_remlPB <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_all, method = "REML",
                                             
                                             mods = ~ Feedback:Information:SocialComparison:Motivation:MonetaryRewards+0+var_smd)
# calculate I^2
compute_I2(model_remlPB)

EffectSizeSummary_RE2 <- data_frame() 
EffectSizeSummary_RE2 <- rbind(EffectSizeSummary_RE2, tidy3(model_remlPB))
...
EffectSizeSummary_RE2 <- recode_binary_predictors(EffectSizeSumamry_RE2)
EffectSizeSummary_RE2 <- process_order_label(EffectSizeSumamry_RE)
```

```{r meta-analysis of average effect size incl. publication bias}
dfinc_all$se = (dfinc_all$var_smd)^(1/2)

meta_all <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_all, method = "REML")
summary(meta_all)
compute_I2(meta_all)

meta_all_se <-rma.mv(smd, var_smd, mods =  ~ se,
                               random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_all, method = "REML")
summary(meta_all_se)
compute_I2(meta_all_se)

meta_all_smd <-rma.mv(smd, var_smd, mods =  ~ var_smd,
                               random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_all, method = "REML")
summary(meta_all_smd)
compute_I2(meta_all_smd)

dfinc_audit$se = (dfinc_audit$var_smd)^(1/2)

meta_all <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_audit, method = "REML")
summary(meta_all)
compute_I2(meta_all)

meta_all_se <-rma.mv(smd, var_smd, mods =  ~ se,
                               random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_audit, method = "REML")
summary(meta_all_se)
compute_I2(meta_all_se)
...
compute_I2(meta_all_smd)
```
```{r meta-analysis of average effect size for individual interventions}
dfinc_all$author_year <- paste0(dfinc_all$Authors, "(", dfinc_all$`document PY`, ")")

dfinc_social <- dfinc_all %>% filter(treatment == "Social")

meta_social <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_social, method = "REML")
summary(meta_social)
compute_I2(meta_social)

png(file = "meta_social_forest.png", width = 1800, height = 2400, res = 300)
metafor::forest.rma(x = meta_social,header = TRUE, 
                    slab = dfinc_social$effectID,
                    xlim = c(-1, 2),
                    cex = 0.6)
dev.off()

dfinc_motivation <- dfinc_all %>% filter(treatment == "Motivation")

meta_motivation <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_motivation, method = "REML")
summary(meta_motivation)
compute_I2(meta_motivation)

png(file = "meta_motivation_forest.png", width = 1800, height = 1500, res = 300)
metafor::forest.rma(x = meta_motivation,header = TRUE, 
                    slab = dfinc_motivation$effectID,
                    xlim = c(-1, 2),
                    cex = 0.6)
dev.off()

dfinc_monetary <- dfinc_all %>% filter(treatment == "Monetary")

meta_monetary <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_monetary, method = "REML")
summary(meta_monetary)
compute_I2(meta_monetary)

png(file = "meta_monetary_forest.png", width = 1800, height = 1500, res = 300)
metafor::forest.rma(x = meta_monetary,header = TRUE, 
                    slab = dfinc_monetary$effectID,
                    xlim = c(-1, 2),
                    cex = 0.6)
dev.off()

dfinc_information <- dfinc_all %>% filter(treatment == "Information")

meta_information <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_information, method = "REML")
summary(meta_information)
compute_I2(meta_information)

png(file = "meta_information_forest.png", width = 1800, height = 3000, res = 300)
metafor::forest.rma(x = meta_information,header = TRUE, 
                    slab = dfinc_information$effectID,
                    xlim = c(-1, 2),
                    cex = 0.6)
dev.off()

dfinc_feedback <- dfinc_all %>% filter(treatment == "Feedback")

meta_feedback <- rma.mv(smd, var_smd, random = ~ 1|as.factor(documentID)/as.factor(effectID), data = dfinc_feedback, method = "REML")
summary(meta_feedback)
compute_I2(meta_feedback)

png(file = "meta_feedback_forest.png", width = 1800, height = 4500, res = 300)
metafor::forest.rma(x = meta_feedback,header = TRUE, 
                    slab = dfinc_feedback$effectID,
                    xlim = c(-1, 2),
                    cex = 0.6)
dev.off()
...
```{r publication bias test and plot}
library(lmtest)
library(ggrepel)
...
legend_labels <- levels(as.factor(dfinc_all$intervention))
...
# save the plot
dev.copy(png, "funnel_plot.png", width = 480, height = 320)
dev.off()
```
...
```{r MA and NMA forest plots}
# prepare intervention counts to match labels
...
# meta-analysis
EffectSizeSummary_RE <- prep_num_label(EffectSizeSumamry_RE)

ggplot(EffectSizeSummary_RE, aes(y = paste0(label, " [", int_count[label], "]") , x = beta)) +
  geom_point(shape=1,size=2, fill="white") +  # fffect size points
  geom_errorbarh(aes(xmin = lower_lim, xmax = upper_lim), height = 0, color="black") +  # horizontal error bars
  theme_bw() + 
  labs(x = "Effect Size (Beta)", y = "Intervention") + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") + 
  theme (axis.text.x = element_text(hjust = 0.5),
         panel.grid = element_blank())+ 
  xlim(-0.5,1.5) +
    geom_text(aes(
  label = significance, x = 1), hjust = 0, size = 3)
# ggsave("ma-forest.png", width = 7, height = 5)

EffectSizeSummary_RE2 <- prep_num_label(EffectSizeSummary_RE2)

ggplot(EffectSizeSummary_RE2 %>% filter(label!="var_smd"), aes(y = paste0(label, " [", int_count[label], "]") , x = beta)) +
  ...
  xlim(-0.5, 1.5) +
    geom_text(aes(
    label = significance, x = 1), hjust = 0, size = 3)
# ggsave("ma-forestPB.png", width = 7, height = 5)
...
EffectSizeSummary_NMA <- nma_result$Summary_Results %>%
  mutate(
    label = str_remove(Treatment, "^d\\.Control\\."), 
    beta = Mean,
    # split the CrI into lower and upper bounds
    lower_lim = as.numeric(str_extract(X95..CrI, "(?<=\\().*?(?=,)")),
    upper_lim = as.numeric(str_extract(X95..CrI, "(?<=,).*?(?=\\))")),
    p_value = as.numeric(p_value)
  )

EffectSizeSummary_NMA <- prep_num_label(EffectSizeSummary_NMA)

ggplot(EffectSizeSummary_NMA %>% filter(label != "sd.d"), aes(y = paste0(label, " [", int_count[label], "]"), x = beta)) +
  ...
  xlim(-0.5, 1.5) +
  geom_text(aes(label = significance, x = 1), hjust = 0, size = 3)
ggsave("nma-forest.png", width = 7, height = 5)
```
...
#save the plot
ggsave("20250613-results/nma-ma-sucra.png", plot = p, width = 8, height = 8, dpi = 300)
```
...
# 235 - Matsukawa 2018 also report VERY low SEs compared to the size of coefficients, which leads to very large t-stats
...
Main part of analysis is over here, but there are some extra tests below that you can run.
...
```{r analysis with audit and dynamic pricing separate - process intervention variables}
...
model_audit <- rma.mv(smd, var_smd, random = ~ 1| as.factor(documentID)/as.factor(effectID), data = dfinc_audit, method = "REML",
                                             mods = ~ Feedback:Information:HomeAudit:SocialComparison:Motivation:MonetaryRewards:DynamicPricing+0)
                                       
compute_I2(model_audit)
  
EffectSizeSummary_RE_audit <- data_frame() 
EffectSizeSummary_RE_audit <- rbind(EffectSizeSummary_RE_audit, tidy3(model_audit))
...
model_auditPB <- rma.mv(smd, var_smd, random = ~ 1| as.factor(documentID)/as.factor(effectID), data = dfinc_audit, method = "REML",
                                             
                                             mods = ~ Feedback:Information:HomeAudit:SocialComparison:Motivation:MonetaryRewards:DynamicPricing+0+var_smd)

compute_I2(model_auditPB)
...
# create plot
ggplot(df, aes(y = paste0(label, " [", int_count_audit[label], "]"), x = beta)) +
  geom_point(shape = 1, size = 2) +
  geom_errorbarh(aes(xmin = lower_lim, xmax = upper_lim), height = 0) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_text(aes(
    label = ifelse(pvalues < 0.001, paste0(round(beta, 2), "***  (", round(lower_lim, 2), ",", round(upper_lim, 2), ")"),
             ifelse(pvalues < 0.01, paste0(round(beta, 2), "**  (", round(lower_lim, 2), ",", round(upper_lim, 2), ")"),
...
ggsave("ma-forest-audit.png", width = 7, height = 5)
```
...
# create plot
ggplot(df2, aes(y = paste0(label, " [", int_count_audit[label], "]"), x = beta)) +
  geom_point(shape = 1, size = 2) +
  geom_errorbarh(aes(xmin = lower_lim, xmax = upper_lim), height = 0) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_text(aes(
...
ggsave("ma-forest-auditPB.png", height = 7, width = 5)
```

```{r analysis with audit separate - network meta-analysis incl. plots}
### NMA ###

dfinc_audit <- dfinc_audit %>% filter(!is.na(treatment))
...
print(p_audit)
#save the plot
ggsave("nma-maPB-sucra-audit.png", plot = p, width = 8, height = 8, dpi = 300)
```
...
table(dfinc_all$cat_criterion2)
...
ggsave("/1cat-low.png", width = 6, height = 6, dpi = 300)
```
...
ggsave("2cat-med.png", width = 6, height = 6, dpi = 300)
```
...
ggsave("7cat-high.png", width = 6, height = 6, dpi = 300)
```
...
ggsave("7cat-lowmed.png", width = 6, height = 6, dpi = 300)
```
...
#ggsave("lowmedoverallrob-forest.png", width = 6, height = 6, dpi = 300)
```
...
ggsave("overallrob-forest.png", width = 6, height = 6, dpi = 300)
```
